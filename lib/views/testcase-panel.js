...
export default class TestcasePanel {
  ...
  addTestcase(name, kill) {
    ...
    `;
    testcase.firstElementChild.lastElementChild.onclick = kill;
  }

  resolveTestcase(id, input, output, msg, className) {
    let child = this.element.children[id];
    let header = child.firstElementChild;
    ...
    message.className = 'borto-pack-testcase-message ' + className;
