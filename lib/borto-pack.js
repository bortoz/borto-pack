/** @babel */

import { CompositeDisposable, Notification } from 'atom';
import child_process from 'child_process';
import fs from 'fs';
import path from 'path';
import pump from 'pump';
import through from 'through2';
import TestcasePanel from './views/testcase-panel';

export default class BortoPackage {
  debugMode = true;

  constructor() {
    this.ext = process.platform.startsWith('win32') ? '.exe' : '';
    this.loaderPath = path.join(__dirname, 'loader.py');
    this.pendingProcesses = [];
  }

  addNotification(type, message, options, leave) {
    Object.assign(options || {}, {
      dismissable: true,
      signature: 'borto-pack'
    });
    let notification = atom.notifications.addNotification(new Notification(type, message, options));
    if (!leave) {
      setTimeout(() => notification.dismiss(), 3000);
    }
    return notification;
  }

  async update() {
    for (const notification of atom.notifications.getNotifications()) {
      if (notification.options.signature === 'borto-pack') {
        notification.dismiss();
      }
    }
    this.editor = atom.workspace.getActiveTextEditor();
    this.fileType = this.editor.getGrammar().name;
    this.file = this.editor.getBuffer().file;
    if (!this.file) {
      this.addNotification('error', '<strong>File not found</strong><br>Save before running a command');
      throw new Error('File not found');
    }
    try {
      await this.editor.save();
    } catch (error) {
      this.addNotification('error', '<strong>Could not save the file</strong>');
      throw error;
    }
    this.filePath = this.file.path;
    this.info = path.parse(this.filePath);
    this.dir = this.info.dir;
    this.name = this.info.name;
  }

  async build() {
    switch (this.fileType) {
      case 'C':
        var command = atom.config.get('borto-pack.cCompiler');
        var args = ['-o', path.join(this.dir, this.name), ...atom.config.get('borto-pack.cOptions').split(' '), this.filePath];
        break;
      case 'C++':
        var command = atom.config.get('borto-pack.cppCompiler');
        var args = ['-o', path.join(this.dir, this.name), ...atom.config.get('borto-pack.cppOptions').split(' '), this.filePath];
        break;
      case 'Java':
        var command = atom.config.get('borto-pack.javaCompiler');
        var args = ['-g', this.filePath];
        break;
      default:
        this.addNotification('error', '<strong>Unrecognized file type</strong>');
        throw new Error('Unrecognized file type');
    }
    let options = {
      cwd: this.dir
    };
    await new Promise((resolve, reject) => {
      child_process.execFile(command, args, options, (error, stdout, stderr) => {
        stderr = String(stderr);
        if (error) {
          let options = {
            detail: stderr
          };
          this.addNotification('error', '<strong>Compilation failed</strong>', options, true);
          reject(error);
        } else {
          if (stderr.length === 0) {
            this.addNotification('success', '<strong>Compilation successful</strong>');
          } else {
            let options = {
              detail: stderr
            };
            this.addNotification('warning', '<strong>Compilation successful</strong>', options);
          }
          resolve();
        }
      });
    });
  }

  async run() {
    let pyCompiler = atom.config.get('borto-pack.pyCompiler');
    switch (this.fileType) {
      case 'C':
      case 'C++':
        var exeFile = path.join(this.dir, this.name).concat(this.ext);
        var exeCommand = exeFile;
        break;
      case 'Python':
        var exeFile = this.filePath;
        var exeCommand = `${pyCompiler} ${exeFile}`;
        break;
      case 'Java':
        var exeFile = path.join(this.dir, this.name).concat('.class');
        var exeCommand = `java ${this.name}`;
        break;
      default:
        this.addNotification('error', '<strong>Unrecognized file type</strong>');
        throw new Error('Unrecognized file type');
    }
    await new Promise((resolve, reject) => {
      fs.access(exeFile, error => {
        if (error) {
          this.addNotification('error', '<strong>Executable file not found</strong>');
          reject(error);
        } else {
          resolve();
        }
      });
    });
    let terminal = atom.config.get('borto-pack.terminal');
    if (terminal === 'conhost') {
      var command = `start "${this.name}" ${pyCompiler} ${this.loaderPath} ${exeCommand}`;
    } else if (terminal === 'Windows Terminal') {
      var command = `wt.exe ${pyCompiler} ${this.loaderPath} ${exeCommand}`;
    } else if (terminal === 'GNOME') {
      var command = `gnome-terminal -t ${this.name} -- ${pyCompiler} ${this.loaderPath} ${exeCommand}`;
    } else if (terminal === 'xterm') {
      var command = `xterm -e ${pyCompiler} ${this.loaderPath} ${exeCommand}`;
    } else if (terminal === 'Konsole') {
      var command = `konsole -e ${pyCompiler} ${this.loaderPath} ${exeCommand}`;
    } else if (terminal === 'Xfce') {
      var command = `xfce4-terminal --command ${pyCompiler} ${this.loaderPath} ${exeCommand}`;
    } else if (terminal === 'Pantheon') {
      var command = `pantheon-terminal -e ${pyCompiler} ${this.loaderPath} ${exeCommand}`;
    } else if (terminal === 'URxvt') {
      var command = `urxvt -e ${pyCompiler} ${this.loaderPath} ${exeCommand}`;
    } else if (terminal === 'MATE') {
      var command = `mate-terminal --command ${pyCompiler} ${this.loaderPath} ${exeCommand}`;
    }
    let options = {
      cwd: this.dir
    };
    await new Promise((resolve, reject) => {
      child_process.exec(command, options, error => {
        if (error) {
          this.addNotification('error', '<strong>Error while executing file</strong>', {}, true);
          reject(error);
        } else {
          resolve();
        }
      });
    });
  }

  async buildAndRun() {
    await this.build();
    await this.run();
  }

  async multiRun() {
    if (this.pendingProcesses.length > 0) {
      let notification = this.addNotification('error', '<strong>Pending running processes</strong>', {
        description: 'There are pending processes, kill these processes to continue',
        buttons: [{
          text: 'Kill all',
          onDidClick: () => {
            this.pendingProcesses.forEach(child => child.kill());
            notification.dismiss();
          }
        }]
      }, true);
      return;
    }
    let args = [];
    switch (this.fileType) {
      case 'C':
      case 'C++':
        var exeFile = path.join(this.dir, this.name).concat(this.ext);
        var command = exeFile;
        break;
      case 'Python':
        var exeFile = this.filePath;
        var command = atom.config.get('borto-pack.pyCompiler');
        args = [exeFile];
        break;
      case 'Java':
        var exeFile = path.join(this.dir, this.name).concat('.class');
        var command = 'java';
        args = [this.name];
        break;
      default:
        this.addNotification('error', '<strong>Unrecognized file type</strong>');
        throw new Error('Unrecognized file type');
    }
    let options = {
      cwd: this.dir
    };
    await new Promise((resolve, reject) => {
      fs.access(exeFile, error => {
        if (error) {
          this.addNotification('error', '<strong>Executable file not found</strong>');
          reject(error);
        } else {
          resolve();
        }
      });
    });
    let inputFolder = path.join(this.dir, 'input/');
    let inputFiles = await new Promise((resolve, reject) => {
      fs.readdir(inputFolder, (error, files) => {
        if (error) {
          this.addNotification('error', `<strong>Could not open 'input' directory</strong>`);
          reject(error);
        } else {
          resolve(files);
        }
      });
    });
    let outputFolder = path.join(this.dir, 'output/');
    await new Promise((resolve, reject) => {
      fs.readdir(outputFolder, (error, files) => {
        if (error) {
          reject(error);
        } else {
          resolve(files.map(file => path.join(this.dir, 'output', file)));
        }
      });
    }).then(
      files => {
        return Promise.all(
          files.map(file => {
            return new Promise((resolve, reject) => {
              fs.access(file, err => {
                if (err) {
                  reject(err);
                } else {
                  resolve();
                }
              });
            }).then(
              () => {
                return new Promise((resolve, reject) => {
                  fs.unlink(file, err => {
                    if (err) {
                      this.addNotification('error', `<strong>Could not delete '${file}'</strong>`);
                      reject(err);
                    } else {
                      resolve();
                    }
                  });
                });
              },
              () => {
                return Promise.resolve();
              }
            );
          })
        );
      },
      () => {
        return new Promise((resolve, reject) => {
          fs.mkdir(outputFolder, error => {
            if (error) {
              this.addNotification('error', `<strong>Could not open 'output' directory</strong>`);
              reject(error);
            } else {
              resolve();
            }
          });
        });
      }
    );
    let pane = atom.workspace.paneForURI(TestcasePanel.TESTCASE_PANEL_URI);
    if (pane) {
      testcasePanel = pane.items.find(x => x instanceof TestcasePanel);
      testcasePanel.clearTestcase();
    } else {
      testcasePanel = await atom.workspace.open(TestcasePanel.TESTCASE_PANEL_URI);
    }
    let timeLimit = atom.config.get('borto-pack.multiRunTimeout');
    for (const [id, inputFile] of inputFiles.entries()) {
      let inputPath = path.join(this.dir, 'input', inputFile);
      let outputFile = inputFile.replace(/^input/, 'output').replace(/\.in$/, '.out');
      let outputPath = path.join(this.dir, 'output', outputFile);
      let match = /^input[-_.]?(.+)\..+?$/.exec(inputFile) || /^(.+)\..+?$/.exec(inputFile);
      let title = 'Testcase'.concat(match ? ` #${match[1]}` : '');
      let streamInput = fs.createReadStream(inputPath);
      let streamOutput = fs.createWriteStream(outputPath);
      let inputData = '';
      let outputData = '';
      let errorData = '';
      let child = child_process.spawn(command, args, options);
      let timestamp = Date.now();
      let timeLimitExceeded = false;
      if (timeLimit > 0) {
        setTimeout(() => {
          timeLimitExceeded = true;
          child.kill();
        }, timeLimit);
      }
      testcasePanel.addTestcase(title, () => {
        child.kill();
      });
      pump(
        streamInput,
        through((chunk, enc, callback) => {
          inputData = (inputData + chunk).slice(0, 512);
          callback(null, chunk);
        }),
        child.stdin
      );
      pump(
        child.stdout,
        through((chunk, enc, callback) => {
          outputData = (outputData + chunk).slice(0, 512);
          callback(null, chunk);
        }),
        streamOutput
      );
      pump(
        child.stderr,
        through((chunk, enc, callback) => {
          errorData = (errorData + chunk).slice(0, 512);
          callback();
        }),
      );
      child.on('exit', (code, signal) => {
        let time = ((Date.now() - timestamp) / 1000).toFixed(2) + 's';
        if (timeLimitExceeded) {
          testcasePanel.resolveTestcase(id, inputData, errorData, 'TLE', 'error');
        } else if (signal || code) {
          testcasePanel.resolveTestcase(id, inputData, errorData, signal || 'RTE', 'error');
        } else {
          testcasePanel.resolveTestcase(id, inputData, outputData, time, 'success');
        }
        this.pendingProcesses = this.pendingProcesses.filter(c => c.pid !== child.pid);
      });
      this.pendingProcesses.push(child);
    }
  }

  async beautify() {
    switch (this.fileType) {
      case 'C':
      case 'C++':
        var command = 'clang-format';
        var args = ['-style', `{BasedOnStyle: ${atom.config.get('borto-pack.cppBeautify')}, IndentWidth: ${this.editor.getTabLength()}, AllowShortIfStatementsOnASingleLine: true}`, '-i', this.filePath];
        break;
      case 'Python':
        var command = 'yapf';
        var args = ['--style', `{based_on_style: ${atom.config.get('borto-pack.pyBeautify')}, indent_width: ${this.editor.getTabLength()}}`, '-i', this.filePath];
        break;
      case 'Java':
        var command = 'java';
        var args = ['-jar', path.join(__dirname, 'java-format', 'google-java-format-1.7-all-deps.jar'), '-i', this.filePath]
        break;
      default:
        this.addNotification('error', '<strong>Unrecognized file type</strong>');
        throw new Error('Unrecognized file type');
    }
    await new Promise((resolve, reject) => {
      child_process.execFile(command, args, error => {
        if (error) {
          this.addNotification('error', `<strong>Unable to beautify current file</strong>`);
          reject(error);
        } else {
          resolve();
        }
      });
    });
  }

  switchDebugMode() {
    this.debugMode = !this.debugMode;
  }
}
