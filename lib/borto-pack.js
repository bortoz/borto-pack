...
export default class BortoPackage {
  ...
  async multiRun() {
    ...
    inputFiles.forEach(async inputFile => {
      ...
      let child = child_process.spawn(command, args, childOptions);
      let input = '', output = '';
      streamInput.pipe(child.stdin);
      ...
      try {
        await new Promise((resolve, reject) => {
          streamInput.on('error', error => {
            atom.notifications.addError(`<strong>Could not read from ${inputFile}</strong>`);
            reject(error);
          });
          streamOutput.on('error', error => {
            atom.notifications.addError(`<strong>Could not write to ${outputFile}</strong>`);
            reject(error);
          });
          child.on('exit', (code, signal) => {
            if (signal) {
              atom.notifications.addError(`<strong>Process killed on: ${inputFile}</strong>`);
              reject('Process killed');
            } else if (code) {
