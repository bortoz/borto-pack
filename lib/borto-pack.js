...
function parseErrors(log) {
  let messages = [];
  for (let match of matchAll(log, /(.*):\s+(undefined reference to.*|could not read symbols.*)/g)) {
    messages.push({
  ...
  for (let match of matchAll(log, /(.*):\s+(note|warning|error|fatal error):\s+(.*)/g)) {
    messages.push({
      file: parseFilename(match[1]),
      type: match[2],
      text: match[3],
...
export async function run(e) {
  ...
  switch (terminal) {
    ...
    case 'GNOME':
      ...
      break;
    case 'xterm':
      var command = `xterm -e ${pyCompiler} ${loaderPath} ${exeCommand}`;
      break;
    case 'Konsole':
      var command = `konsole -e ${pyCompiler} ${loaderPath} ${exeCommand}`;
      break;
    case 'Xfce':
      var command = `xfce4-terminal --command ${pyCompiler} ${loaderPath} ${exeCommand}`;
      break;
    case 'Pantheon':
    ...
    case 'URxvt':
      ...
      break;
    case 'MATE':
      var command = `mate-terminal --command ${pyCompiler} ${loaderPath} ${exeCommand}`;
      break;
  }
  let options = {
    cwd: dir
