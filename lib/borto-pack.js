...
export default class BortoPackage {
  ...
  async multiRun() {
    ...
    let inputFolder = await new Promise((resolve, reject) => {
      fs.readdir(this.dir, (error, files) => {
        if (error) {
          atom.notifications.addError('<strong>Could not open current directory</strong>');
          reject(error);
        } else {
          resolve(files);
        }
      ...
      .forEach(async inputFile => {
        ...
        let child = child_process.execFile(command, args, childOptions, async (error, stdout) => {
          ...
          if (error) {
            ...
            let match = /^input[-_.]?(.*)\.txt$/.exec(inputFile);
            testcasePanel.addTestcase('Testcase' + (match && match[1] && ` #${match[1]}`), String(input), String(stdout));
          }
        });
